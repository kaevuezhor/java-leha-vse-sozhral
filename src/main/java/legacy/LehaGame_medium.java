...
public class LehaGame_medium extends JPanel implements KeyListener, Runnable {
    ...
    private class Food {
        ...
        private FoodType getRandomFoodType() {
            ...
            if (r < 0.25f) return FoodType.HEALTHY;   // 25%
            if (r < 0.50f) return FoodType.JUNK;       // 25%
            if (r < 0.65f) return FoodType.ANTIDOTE;  // 15%
            if (r < 0.85f) return FoodType.ENERGY;    // 20%
            return FoodType.POISON;                  // 15%
        }
    }

    enum FoodType { HEALTHY, JUNK, POISON, ENERGY, ANTIDOTE }

    public LehaGame_medium(JFrame frame) {
    ...
    private void loadSounds() {
        try {
            // Получаем URL ресурса из папки resources
            URL soundUrl = getClass().getResource("/eat.wav");
            if (soundUrl == null) {
                throw new IOException("Файл eat.wav не найден в ресурсах!");
            }
    ...
    private void updateSpeed() {
        float minSpeed = 0.2f;
        float sizeRatio = (float) playerSize / MAX_SIZE;
        speedMultiplier = Math.max(minSpeed, 1.0f - sizeRatio * 0.8f);
    }
    ...
    public void run() {
        while (gameRunning && totalCalories > 0 && lives > 0
                && totalCalories < 3000 && playerSize <= MAX_SIZE) {
            checkCollisions();
            checkTime();
            repaint();
            try { Thread.sleep(50); }
    ...
    private void applyFoodEffect(FoodType type) {
        switch (type) {
            ...
            case JUNK:
                ...
                break;
            case POISON:
                totalCalories = Math.max(0, totalCalories - 200);
                playerSize = Math.max(MIN_SIZE, playerSize - 25);
                break;
            case ENERGY:
                totalCalories += 600;
                break;
        ...
        playerSize = Math.min(playerSize, MAX_SIZE + 1);
        playerSize = Math.max(playerSize, MIN_SIZE);
        updateSpeed();
    }
    ...
    public void keyPressed(KeyEvent e) {
        int baseStep = isTurbo ? 25 : 15;
        int step = (int)(baseStep * speedMultiplier);
        int calorieCost = isTurbo ? 5 : 2;
    ...
    public static void main(String[] args) {
        ...
        LehaGame_medium game = new LehaGame_medium(frame);
        frame.add(game);
        frame.pack();
